...
class PaymentIntentCreateRequest(BaseModel):
    ...
    payment_method_data: Optional[PaymentMethodCreateRequest] = Field(default=None, description="指定した場合、PaymentMethodの作成に使用されます。新しいPaymentMethodは、PaymentIntentのpayment_methodプロパティに表示されます。")
    payment_method_options: Optional[PaymentMethodOptionsRequest] = Field(default=None, description="このPaymentIntentに固有の支払い方法の設定")
    payment_method_types: Optional[List[PaymentMethodTypes]] = Field(default=None, description="このPaymentIntentで使用できる支払い方法の種類（カードなど）のリストです。 指定しない場合は、PAY.JPは支払い方法の設定から利用可能な支払い方法を動的に表示します。")
    ...
    customer: Optional[StrictStr] = Field(default=None, description="このPaymentIntentに属する顧客のID（存在する場合）。この顧客以外にすでに紐づけられている支払い方法はこのPaymentIntentでは使用できません。")
    confirm: Optional[StrictBool] = Field(default=False, description="「true」に設定すると、このPaymentIntentを直ちに確定しようと試みます。このパラメーターのデフォルトは「false」です。")
    capture_method: Optional[CaptureMethod] = Field(default=None, description="支払いの確定方法を指定します。  | 指定できる値 | |:---| | **automatic**: 顧客が支払いを承認すると、自動的に確定させます。 | | **automatic_async**: (デフォルト) 顧客が支払いを承認すると、非同期的に確定させます。レイテンシが改善されるため、capture_method=automaticよりも推奨されます。詳細については、統合ガイドをお読みください。 | | **manual**: 顧客が支払いを承認すると一旦確定を保留し、後で Capture API を使用して確定します。（すべての支払い方法がこれをサポートしているわけではありません）。 |")
    ...
    setup_future_usage: Optional[Usage] = Field(default=None, description="このPaymentIntentの支払い方法で将来の支払いを行う意図があることを示します。<br><br>PaymentIntent に Customer を指定した場合、このパラメータを使って PaymentIntent を確定できます。その後、顧客が必要な操作を完了すると、支払い方法を Customer に紐付けることが可能です。また、Customer を指定しない場合でも、取引が完了した後に支払い方法を Customer に紐付けることはできます。")
