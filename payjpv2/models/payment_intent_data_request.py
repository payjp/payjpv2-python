...
class PaymentIntentDataRequest(BaseModel):
    ...
    setup_future_usage: Optional[StrictStr] = Field(default=None, description="この PaymentIntent に設定されている支払い方法で今後決済を行うかの設定です。<br><br> PaymentIntent に Customer を指定した場合、このパラメータを使って PaymentIntent を確定できます。 その後、顧客が必要な操作を完了すると、支払い方法を Customer に紐付けることが可能です。また、Customer を指定しない場合でも、取引が完了した後に支払い方法を Customer に紐付けることはできます。")
