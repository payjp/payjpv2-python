...
class SetupIntentCreateRequest(BaseModel):
    ...
    """ # noqa: E501
    confirm: Optional[StrictBool] = Field(default=False, description="SetupIntent をすぐに確定しようとする場合に `true` を設定します。このパラメーターのデフォルト値は `false` です。カードが登録済みの支払い方法である場合は、追加の認証が必要な場合に備えて `return_url` を指定できます。")
    customer: Optional[StrictStr] = Field(default=None, description="この SetupIntent が属する顧客の ID。SetupIntent に PaymentMethod が設定されている場合、SetupIntent の設定が成功するとその PaymentMethod は顧客に紐付きます。別の顧客に紐付いている PaymentMethod をこの SetupIntent で使用することはできません。")
