...
class SetupIntentResponse(BaseModel):
    ...
    object: Optional[StrictStr] = 'setup_intent'
    ...
    client_secret: StrictStr = Field(description="この SetupIntent のクライアントシークレットです。フロントエンドで公開鍵と合わせて使用し、SetupIntent の取得や支払い処理を行います。**この値はこの SetupIntent の支払いを行う顧客以外へ公開しないでください。")
    ...
    payment_method_types: List[StrictStr] = Field(description="この SetupIntent で使用できる支払い方法の種類（カードなど）のリストです。 指定しない場合、ダッシュボードで利用可能な状態にしている支払い方法が自動的に設定されます。")
    status: SetupIntentStatus = Field(description="この SetupIntent のステータスです。<a href=\"https://docs.pay.jp/v2/setup_intents#status\" target=\"_blank\">ステータスの詳細についてはこちらをご覧ください。</a>  | 値 | |:---| | **requires_payment_method**: 支払い方法が必要です。 | | **requires_confirmation**: 確認が必要です。 | | **requires_action**: 顧客のアクションが必要です。 | | **processing**: 処理中です。 | | **succeeded**: 成功しました。 | | **canceled**: キャンセルされました。 |")
    next_action: Optional[Dict[str, Any]] = None
    return_url: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["id", "object", "created_at", "updated_at", "livemode", "client_secret", "customer", "description", "metadata", "payment_method", "payment_method_options", "payment_method_types", "status", "next_action", "return_url"]
    ...
    def object_validate_enum(cls, value):
        ...
        if value not in set(['setup_intent']):
            raise ValueError("must be one of enum values ('setup_intent')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "object": obj.get("object") if obj.get("object") is not None else 'setup_intent',
