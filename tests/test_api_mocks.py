...
class TestPaymentIntentsApiMock:
    """Test Payment Intents API with mocked responses."""

    @patch('payjpv2.api_client.ApiClient.response_deserialize')
    @patch('payjpv2.api_client.ApiClient.call_api')
    def test_create_payment_intent_success(self, mock_call_api, mock_deserialize, api_client, sample_payment_intent_data):
        """Test successful payment intent creation."""
        payment_intents_api = PaymentIntentsApi(api_client)
        
        mock_rest_response = Mock()
        mock_rest_response.status = 200
        mock_rest_response.data = json.dumps(sample_payment_intent_data).encode()
        mock_rest_response.read = Mock()
        mock_call_api.return_value = mock_rest_response
        
        mock_api_response = Mock()
        mock_api_response.data = sample_payment_intent_data
        mock_deserialize.return_value = mock_api_response
        
        request = PaymentIntentCreateRequest(amount=1000)
        response = payment_intents_api.create_payment_intent(request)
    ...
    @patch('payjpv2.api_client.ApiClient.response_deserialize')
    @patch('payjpv2.api_client.ApiClient.call_api')
    def test_retrieve_payment_intent_success(self, mock_call_api, mock_deserialize, api_client, sample_payment_intent_data):
        ...
        response = payment_intents_api.retrieve_payment_intent("pi_test123")
        
        # Verify API call
        mock_call_api.assert_called_once()
        mock_deserialize.assert_called_once()
        
        assert response['id'] == 'pi_test123'
...
class TestModelIntegration:
    ...
    def test_payment_intent_model_creation(self):
        ...
        assert request.description == "Test payment intent"
