...
class TestSDKIntegration:
    ...
    def test_payment_intent_workflow(self, api_client, sample_payment_intent_data):
        """Test payment intent creation and confirmation workflow."""
        payment_intents_api = PaymentIntentsApi(api_client)
        
        with patch.object(payment_intents_api, 'retrieve_payment_intent') as mock_retrieve:
            # Mock payment intent retrieval
            mock_response = Mock()
            mock_response.data = sample_payment_intent_data
            mock_retrieve.return_value = mock_response
            
            # Retrieve payment intent
            retrieved_pi = payment_intents_api.retrieve_payment_intent("pi_test123")
            
            # Verify workflow
            assert retrieved_pi.data['amount'] == 1000
            assert retrieved_pi.data['id'] == 'pi_test123'
